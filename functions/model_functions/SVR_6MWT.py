...
def finding_models(all_data):
    ...
    # from this matrix can be extracted the labels (i.e., _6MWT_POST)
    
    _6MWT_POST = all_data['6MWT_POST']
    data = all_data.drop(['6MWT_POST'], axis = 1)
    ...
    # Creating bins of _6MWT_POST
    bins = pd.qcut(_6MWT_POST, q=5, labels=False)  # Quantile binning into 5 groups

    x_train, x_test, y_train, y_test = train_test_split(selected_data, _6MWT_POST, test_size=0.25, random_state=72, stratify=bins)    
    ...
    for kernel in kernels:
        ...
        optimizer = BayesianOptimization(f = svr_model, pbounds = pbounds, verbose=3)
...
def prediction_6MWT(X_pp, all_data):  
    ...
    _6MWT_POST_scaled = scaler_y.fit_transform(_6MWT_POST.array.reshape(-1,1))
    ...
    # Training the model
    SVR_best.fit(data_scaled, _6MWT_POST_scaled)
