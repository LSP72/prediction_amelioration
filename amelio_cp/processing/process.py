...
class Process:
    ...
    def calculate_ROM(df:DataFrame):
        ...
        df['ROM_Hip_Sag'] = df['Max_Hip_flx/ext'] - \
            df['Min_Hip_flx/ext']
        df['ROM_Hip_Frontal'] = df['Max_Hip_abd/add'] - \
            df['Min_Hip_abd/add']
        df['ROM_Hip_Trns'] = df['Max_Hip_int/ext rot'] - \
            df['Min_Hip_int/ext rot']
        df['ROM_Knee_Sag'] = df['Max_Knee_flx/ext'] - \
            df['Min_Knee_flx/ext']
        df['ROM_Ankle_Sag'] = df['Max_Ankle_flx/ext'] - \
            df['Min_Ankle_flx/ext']

        # TODO: putting the following lines in the function that will do the 
              # feature engineering  
        df.drop(['Min_Knee_abd/add', 'Min_Knee_int/ext rot', 'Min_Ankle_abd/add', 'Min_Ankle_int/ext rot',
    ...
    def load_data(self, data_dir: str, output_dir: str, gps_path: str, demographic_path: str, separate_legs: bool = True):
        ...
        """
        # file info
        measurements = ['pctSimpleAppuie',
                        'distFoulee', 'vitCadencePasParMinute']
