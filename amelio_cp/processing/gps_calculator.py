...
def calculate_gps(data, reference, separate_legs: bool):
    ...
    if separate_legs == False:
        ...
        for i in range(len(reference)):
            right_diffrences = right_leg.values[:, i] - reference[i]
            left_diffrences = left_leg.values[:, i] - reference[i]
            right_GVS = np.sqrt(np.mean(right_diffrences**2))
            left_GVS = np.sqrt(np.mean(left_diffrences**2))
            right_all_GVS.append(right_GVS)
            left_all_GVS.append(left_GVS)
...
input_directory = r"datasets/sample_2/raw_data/gps_data"
...
output_path = r"datasets/sample_2/raw_data/gps_data/gps_cache/"

reference = pd.read_csv(r"datasets/sample_2/raw_data/gps_data/average.csv")
...
GPS_output.to_csv(r"datasets/sample_2/raw_data/gps_data/gps.csv")
