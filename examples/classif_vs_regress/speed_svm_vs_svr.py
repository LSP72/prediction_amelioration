...
ClassifierMetrics.conf_matrix(SVC_vit.y_test, y_pred_vit_classif, class_names=["Non-Responder", "Responder"], title="Confusion Matrix for speed classification")
...
delta_VIT_true = [1 if SVR_vit.y_test.iloc[i] - SVR_vit.X_test['VIT_PRE'].iloc[i] > 0.1 else 0 for i in range(len(SVR_vit.X_train))]

ClassifierMetrics.conf_matrix(SVR_vit.y_test, y_pred_vit_reg, class_names=["Non-Responder", "Responder"], title="Confusion Matrix for speed classification")
